...
export const MYTEST_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => MyTestCompleteComponent),
    multi: true
};
...
@Component({
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="firstName" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <ng-template let-myTest pTemplate="item">
                <myTest-line [myTest]="myTest"></myTest-line>
            </ng-template>
        </p-autoComplete>
	`,
	selector: 'myTest-auto-complete',
    providers: [MYTEST_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class MyTestCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: MyTest = null;

    public suggestions : MyTest[] = [];

    //Placeholders for the callbacks
    private _onTouchedCallback: () => void = () => {};
    private _onChangeCallback: (_:any) => void = () => {};

    constructor(private myTestService : MyTestService, private messageService : MessageService) {
    }
    ...
    complete(event:any) {
        this.myTestService.complete(event.query).
            subscribe(
